...
export default function Register() {
    ...
    const handleRegister = async (e: React.FormEvent) => {
        ...
        try {
            // Moderation Check
            const safety = await checkContentSafety(name);

            if (safety.flagged) {
                // Block High Severity
                // We don't have a UID yet, so we log "Registration Attempt"
                await reportViolation("registration_attempt", name, name, safety.reason || "Vile username", "Registration (Blocked)");
                setError(`Name unavailable: ${safety.reason || "Violates safety guidelines"}`);
                setLoading(false);
                return;
            }

            if (safety.severity === "low") {
                // Report Low Severity (Background)
                reportViolation("registration_attempt", name, name, safety.reason || "Low severity username", "Registration (Allowed)");
            }
